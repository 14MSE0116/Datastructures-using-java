...
public class Dp {
    ...
    //fractional knapsack
    static double fractionalknapsack(int values[], int weights[], int cap) {
    ...
    static class pair implements Comparable<pair> {
        ...
        pair(int val, int wt) {
            this.val = val;
            this.wt = wt;
            this.frac = this.val / this.wt * 1.0;
        }
    ...
    //catalan number->No of bst's,mountains and valleys
            ...
            for (int i = 0, j = gap; j < n; i++, j++) {
                if (gap == 0) {
                    ...
                    if (s.charAt(i) == s.charAt(j)) {
                        //count=prefix+suffix+1
                        dp[i][j] = dp[i][j - 1] + dp[i + 1][j] + 1;
                    } else {
                        //count=prefix+suffix-mid;
